...
// Scaffold API
const apiPath = path.join(process.cwd(), 'src/app/api/purchasing');
const models = [
  { name: 'purchase_requests', dbTarget: 'purchaseRequests' },
  { name: 'purchase_orders', dbTarget: 'purchaseOrders' },
];
...
// Scaffold UI
const uiPath = path.join(process.cwd(), 'src/app/purchasing');
const pages = [
  {
    folder: 'requests',
    title: 'Yêu cầu mua hàng (PR)',
    desc: 'Quản lý yêu cầu mua vật tư từ xưởng và các bộ phận.',
    icon: 'FileText',
    apiRoute: '/api/purchasing/purchase_requests',
    interfaceName: 'PurchaseRequest',
    fields: [
      { key: 'prNumber', label: 'Mã PR', type: 'text' },
      { key: 'projectId', label: 'ID Dự án', type: 'number' },
      { key: 'requestDate', label: 'Ngày yêu cầu', type: 'text' },
      { key: 'requiredDate', label: 'Ngày cần hàng', type: 'text' },
      { key: 'status', label: 'Trạng thái', type: 'select', options: ['DRAFT', 'SUBMITTED', 'APPROVED', 'REJECTED'] },
  ...
  },
  {
    ...
    desc: 'Quản lý đơn đặt hàng gửi nhà cung cấp.',
    icon: 'ShoppingCart',
    apiRoute: '/api/purchasing/purchase_orders',
    interfaceName: 'PurchaseOrder',
    fields: [
      { key: 'poNumber', label: 'Mã PO', type: 'text' },
      { key: 'supplierId', label: 'ID Nhà cung cấp', type: 'number' },
      { key: 'projectId', label: 'ID Dự án', type: 'number' },
      { key: 'orderDate', label: 'Ngày đặt hàng', type: 'text' },
      { key: 'deliveryDate', label: 'Ngày giao hàng', type: 'text' },
      { key: 'totalAmount', label: 'Tổng tiền', type: 'number' },
      { key: 'status', label: 'Trạng thái', type: 'select', options: ['DRAFT', 'ISSUED', 'PARTIAL', 'RECEIVED', 'CANCELLED'] },
    ]
  }
];
...
console.log('P5 Purchasing Scaffolding completed!');
