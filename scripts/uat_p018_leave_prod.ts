...
async function loginAs(page, username, password = 'password123') {
  await page.goto('https://homepro-manager.vercel.app/login');
...
async function uat_p018() {
  ...
  await managerPage.goto('https://homepro-manager.vercel.app/leave');
  ...
  // Verify Dashboard
  await hrPage.goto('https://homepro-manager.vercel.app/hr');
  await hrPage.waitForSelector('text=BÁO CÁO TỔNG QUAN NHÂN SỰ');
  console.log('   ✅ HR Dashboard loaded');
  
  // Verify Leave
  await hrPage.goto('https://homepro-manager.vercel.app/leave');
  ...
  const res = await workerPage.request.get('https://homepro-manager.vercel.app/api/hr/leave?employeeId=999');
  ...
  await hrPage.goto('https://homepro-manager.vercel.app/attendance');
