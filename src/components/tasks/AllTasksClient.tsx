...
import type { Task, Project, TaskStatus, TaskPriority } from '@/db/schema';
...
export default function AllTasksClient({ initialTasks, projects }: AllTasksClientProps) {
  ...
  // Dynamically aggregate all assignees & categories from tasks + defaults
  const allAssignees = Array.from(new Set([...DEFAULT_ASSIGNEES, ...tasks.map((t) => t.assignee).filter(Boolean)])) as string[];
  const allCategories = Array.from(new Set([...TASK_CATEGORIES, ...tasks.map((t) => t.category).filter(Boolean)])) as string[];
  ...
  return (
    <div>
      {/* Global Datalists for custom typing & selecting */}
      <datalist id="global-assignees-list">
        {allAssignees.map((a) => (
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
              <th style={{ minWidth: 200 }}>Tên công việc (Gõ)</th>
              <th>Dự án</th>
              <th style={{ minWidth: 130 }}>Hạng mục (Gõ/Chọn)</th>
              <th style={{ minWidth: 130 }}>Phụ trách (Gõ/Chọn)</th>
              <th style={{ minWidth: 130 }}>Deadline</th>
              <th style={{ minWidth: 120 }}>Ưu tiên</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              filtered.map((task) => {
                ...
                return (
                  <tr key={task.id} id={`all-task-row-${task.id}`}>
                    {/* 1. Tên công việc */}
                    <td>
                      <input
                    ...
                    <td>
                      {project ? (
                        <Link
                          ...
                          style={{ color: 'var(--color-primary)', fontWeight: 600, fontSize: 12, whiteSpace: 'nowrap' }}
                        >
                          {project.name}
                        </Link>
                      ) : '—'}
                    ...
                    {/* 3. Hạng mục (Editable Input with Datalist) */}
                    <td>
                      <input
                    ...
                    {/* 4. Phụ trách (Editable Input with Datalist - Gõ tên mới tùy ý!) */}
                    <td>
                      <input
                        list="global-assignees-list"
                        className="filter-bar-select"
                        style={{ padding: '4px 8px', fontSize: 12, width: '100%', fontWeight: 600, color: 'var(--color-text)' }}
      ...
      {viewMode === 'list' && tasks.length > 0 && (
        <div className="flex gap-4 mt-4" style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>
          <span>Tổng số công việc: <strong style={{ color: 'var(--color-text)' }}>{tasks.length}</strong></span>
