...
export default function PwrProjectsModal({ onClose, onChanged }: Props) {
  ...
  async function handleDelete(id: number) {
    if (!confirm('Bạn có chắc muốn xóa dự án này khỏi danh sách?')) return;
    try {
      await fetch(`/api/pwr/projects/${id}`, { method: 'DELETE' });
      await fetchProjects();
      onChanged();
    } catch (e) {
      console.error(e);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ margin: 0, fontSize: 18 }}>Quản lý Dự án</h2>
        ...
        <form onSubmit={handleAdd} style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          <input
            ...
            style={{ flex: 1 }}
            placeholder="Nhập tên đơn/dự án..."
            value={newName}
        ...
        <div style={{ maxHeight: 300, overflowY: 'auto' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', color: 'var(--color-text-muted)', padding: 20 }}>Chưa có dự án nào trong danh sách.</div>
          ) : (
