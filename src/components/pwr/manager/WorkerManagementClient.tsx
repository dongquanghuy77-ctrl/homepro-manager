...
export default function WorkerManagementClient() {
  ...
  const handleCreate = async () => {
    if (!form.name||!form.phone||!form.password) { setError('Nh?p d?y d?: Tên, SÐT, M?t kh?u'); return; }
    ...
    setSuccess('Ðã t?o tài kho?n: ' + form.name);
  ...
  const handleEdit = async () => {
    ...
    const d = await r.json();
    if (!r.ok) { setError(d.error||'L?i'); setSaving(false); return; }
    setSuccess('Ðã c?p nh?t'); setEditWorker(null); setSaving(false); fetch_();
  };
    ...
    <div style={{ minHeight:'100vh', background:c.bg, color:'#fff', padding:32 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16, marginBottom:32 }}>
        {[
          ...
          { label:'Ðã hoàn thành task', value:workers.filter(w=>(w.tasksCompleted||0)>0).length, color:c.success },
