    ...
    <div style={{ marginTop:24 }}>
      ...
      {items.length > 0 && (
        <div style={{ marginBottom:10, height:4, background:'rgba(255,255,255,0.06)', borderRadius:999, overflow:'hidden' }}>
          <div style={{ height:'100%', background:pct===100?'#10b981':'#3b82f6', borderRadius:999, transition:'width 0.4s ease', width:pct+'%' }}/>
        </div>
      )}
      ...
      {loading ? <div style={{ color:'#64748b', fontSize:12 }}>Đang tải...</div> : (
        <div>
          {items.map(item => (
            <div key={item.id} className="pwr-fadein"
              ...
              <button onClick={()=>remove(item.id)}
                style={{ background:'none', border:'none', cursor:'pointer', padding:0, color:'#ef4444', opacity:0, transition:'opacity 0.15s' }}
