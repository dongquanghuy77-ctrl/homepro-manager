...
export async function updateEmployeeTransaction(userId: number, payload: UpdateEmployeePayload) {
  return await db.transaction(async (tx) => {
    ...
    if (payload.name !== undefined) updateUsersData.name = payload.name;
    ...
    if (payload.employeeCode !== undefined) updateUsersData.employeeCode = payload.employeeCode;

    if (payload.password) {
      updateUsersData.password = await bcrypt.hash(payload.password, 10);
    }

    // Is this a manual promotion?
    const isManualInput = !oldUser.employeeCode && payload.employeeCode;
    ...
    if (isManualInput) {
      ...
      updateUsersData.departmentId = finalDepartmentId;
      ...
      if (payload.officialSalary !== undefined && payload.officialSalary !== oldUser.officialSalary) {
        ...
        await tx.update(salaryProfiles)
          ...
          .where(and(
            eq(salaryProfiles.userId, userId),
