...
export default function Page() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({"poNumber":"","supplierId":0,"projectId":0,"orderDate":"","deliveryDate":"","totalAmount":0,"status":"DRAFT"});
        loadData();
      }
    } catch (err) {
      console.error(err);
  ...
  async function handleDelete(id: number) {
    ...
    try {
      const res = await fetch(`/api/purchasing/purchase_orders/${id}`, { method: 'DELETE' });
      if (res.ok) loadData();
    } catch (err) {
      console.error(err);
    }
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="text-left py-3 px-4 text-sm font-medium text-gray-500">Mã PO</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">ID Nhà cung cấp</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">ID Dự án</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">Ngày đặt hàng</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">Ngày giao hàng</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">Tổng tiền</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">Trạng thái</th>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4">{item.poNumber}</td><td className="py-3 px-4">{Number(item.supplierId).toLocaleString()}</td><td className="py-3 px-4">{Number(item.projectId).toLocaleString()}</td><td className="py-3 px-4">{item.orderDate}</td><td className="py-3 px-4">{item.deliveryDate}</td><td className="py-3 px-4">{Number(item.totalAmount).toLocaleString()}</td><td className="py-3 px-4"><span className="badge badge-primary">{item.status}</span></td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 md:p-6 overflow-y-auto flex-1">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Mã PO</label>
                <input type="text" className="form-input w-full" value={formData.poNumber as any} onChange={e => setFormData({...formData, poNumber: e.target.value})} required />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">ID Nhà cung cấp</label>
                <input type="number" className="form-input w-full" value={formData.supplierId as any} onChange={e => setFormData({...formData, supplierId: Number(e.target.value)})} required />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" className="form-input w-full" value={formData.orderDate as any} onChange={e => setFormData({...formData, orderDate: e.target.value})} required />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Ngày giao hàng</label>
                <input type="text" className="form-input w-full" value={formData.deliveryDate as any} onChange={e => setFormData({...formData, deliveryDate: e.target.value})} required />
              </div>
              <div>
              ...
              <div>
                ...
                <select className="form-input w-full" value={formData.status as string} onChange={e => setFormData({...formData, status: e.target.value})}>
                  <option value="DRAFT">DRAFT</option><option value="ISSUED">ISSUED</option><option value="PARTIAL">PARTIAL</option><option value="RECEIVED">RECEIVED</option><option value="CANCELLED">CANCELLED</option>
                </select>
              </div>
              </form>
            </div>
