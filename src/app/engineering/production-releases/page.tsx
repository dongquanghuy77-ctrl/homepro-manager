...
interface ProductionReleaseItem {
  id: number;
  designId: number;
  projectId: number;
  status: string;
...
export default function Page() {
  const [data, setData] = useState<ProductionReleaseItem[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const res = await fetch('/api/engineering/production_releases');
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({"designId":0,"projectId":0,"status":"PENDING"});
        loadData();
      }
    } catch (err) {
      console.error(err);
  ...
  async function handleDelete(id: number) {
    ...
    try {
      const res = await fetch(`/api/engineering/production_releases/${id}`, { method: 'DELETE' });
      if (res.ok) loadData();
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Lệnh phát hành bản vẽ chuẩn xuống xưởng.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="text-left py-3 px-4 text-sm font-medium text-gray-500">ID Thiết kế</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">ID Dự án</th><th className="text-left py-3 px-4 text-sm font-medium text-gray-500">Trạng thái</th>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4">{Number(item.designId).toLocaleString()}</td><td className="py-3 px-4">{Number(item.projectId).toLocaleString()}</td><td className="py-3 px-4"><span className="badge badge-primary">{item.status}</span></td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 md:p-6 overflow-y-auto flex-1">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">ID Thiết kế</label>
                <input type="number" className="form-input w-full" value={formData.designId as any} onChange={e => setFormData({...formData, designId: Number(e.target.value)})} required />
              </div>
              <div>
