...
import { CheckSquare, Search, Plus, Trash2 } from 'lucide-react';
...
export default function Page() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const res = await fetch('/api/engineering/approvals');
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({"designId":0,"customerId":0,"status":"PENDING","comments":""});
        loadData();
      }
    } catch (err) {
      console.error(err);
  ...
  async function handleDelete(id: number) {
    ...
    try {
      const res = await fetch(`/api/engineering/approvals/${id}`, { method: 'DELETE' });
      if (res.ok) loadData();
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Theo dõi quá trình khách hàng phê duyệt thiết kế/vật liệu.</p>
        </div>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4">{Number(item.designId).toLocaleString()}</td><td className="py-3 px-4">{Number(item.customerId).toLocaleString()}</td><td className="py-3 px-4"><span className="badge badge-primary">{item.status}</span></td><td className="py-3 px-4">{item.comments}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 md:p-6 overflow-y-auto flex-1">
              ...
              <div>
                ...
                <input type="text" className="form-input w-full" value={formData.comments as any} onChange={e => setFormData({...formData, comments: e.target.value})} required />
              </div>
              </form>
            </div>
